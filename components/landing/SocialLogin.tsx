...
const providerConfig: Record<Provider, { label: string; icon: React.ReactNode; bg: string }> = {
  google: {
  ...
  twitter: {
    ...
    bg: 'bg-black hover:bg-gray-900 text-white border border-white/10',
    icon: (
  ...
  return (
    <div className="w-full max-w-sm mx-auto flex flex-col gap-4 px-4">
      ...
      {(Object.keys(providerConfig) as Provider[]).map((provider) => {
        ...
        return (
          <button
            ...
            className={`flex items-center justify-center gap-3 w-full px-6 py-3.5 rounded-xl font-semibold text-sm transition-all duration-200 ${cfg.bg} disabled:opacity-60 disabled:cursor-not-allowed`}
          >
            {loading === provider ? (
      ...
      <p className="text-center text-muted/60 text-xs mt-4">
        By continuing, you agree to MCV&apos;s Terms of Service and Privacy Policy.
      </p>
    </div>
  )
}
